...
def search_contacts(contacts, search_term):
    ...
    cont = []
    res = search_term.lower()
    for i in contacts:
        if res in i["name"].lower() or res in i["phone"] or res in i["email"]:
            cont.append(i)
    return cont
...
def contact_exists(contacts, name):
    ...
    if find_contact_by_name(contacts, name):
        return True
    else:
        return False
